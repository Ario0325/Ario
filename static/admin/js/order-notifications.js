 ...
 * Order Notifications System v2.0 - Fixed: Aggregated Notifications
 */
...
(function() {
    ...
    let displayedOrderIds = new Set(); // پیگیری سفارش‌های نمایش داده شده
    ...
    async function checkNewOrders() {
        ...
        try {
            ...
            if (data.success && data.orders && data.orders.length > 0) {
                ...
                // فیلتر کردن سفارش‌های جدید (که قبلاً نمایش داده نشده‌اند)
                const newOrders = data.orders.filter(order => !displayedOrderIds.has(order.id));
                
                // افزودن سفارش‌های جدید به مجموعه
    ...
    // تابع نمایش اعلان جمعی
    function showAggregatedNotification(count, orders) {
        // حذف اعلان قبلی اگر وجود دارد
        const existingAggregated = document.getElementById('aggregated-order-notification');
        ...
        notification.id = 'aggregated-order-notification';
        notification.className = 'order-notification aggregated-notification';
        ...
        notification.innerHTML = `
            ...
            <div class="notification-content">
                <div class="notification-title">
                    ${title}
                    <span class="order-count-badge">${count}</span>
                </div>
                <div class="notification-text">
                    ${message}
                </div>
                <div class="notification-meta">
                    <span class="notification-amount">${formatPrice(totalAmount)} تومان</span>
        ...
        // کلیک روی اعلان برای رفتن به سفارش‌ها
